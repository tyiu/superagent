 ...
 */
Request.prototype.use = function(route, fn) {
  // default route to '/'
...
Request.prototype.handle = function(req, out, resp) {
  var self = this
    , stack = self.stack
    , resStack = self.resStack
    , index = 0
    , resp = !!resp;

  function next(err, resNext) {

    // Add the res callback to the stack
    if(resNext && !resp) resStack.push({route: stack[index-1].route, handle: resNext});

    var layer, path, status, c;

    // next callback
    layer = (resp ? resStack : stack)[index++];
